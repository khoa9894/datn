...
\vspace{0.4cm}
Cấu trúc tổng quan của toàn bộ dự án được minh họa như sau:

\vspace{0.3cm}
\begin{forest}
  dir tree
  [dino-math-project/
    [dino-fe/ \textit{(Frontend)}
      [src/
        [app/ \textit{(Routes)}]
        [components/ \textit{(UI, GameComponent)}]
        [apis/]
        [stores/]
        [types/]
      ]
    ...
    [dino\_math\_backend/ \textit{(Backend)}
      [src/
        [controllers/]
        [routes/]
        [model/]
    ...
    [dino-admin-dashboard/ \textit{(Admin)}
      [src/app/
        [api/ \textit{(API clients)}]
        [dashboard/
          [accounts/]
          [study/]
          [finance/]
        ...
        [components/
          [exercises/ \textit{(Editors + Preview)}]
          [ui/ \textit{(shadcn/ui)}]
        ]
      ]
    ]
...
\vspace{0.1cm}
\begin{center}
  \small\textit{Hình: Cấu trúc tổng quan mã nguồn dự án Dino Math}
\end{center}
\vspace{0.3cm}

Sự kết hợp giữa các công nghệ hiện đại này không chỉ giúp dự án vận hành ổn định mà còn tạo tiền đề thuận lợi cho việc triển khai linh hoạt trên hạ tầng đám mây AWS \cite{awsec2} và quản lý thông qua cụm k3s \cite{k3s}. Toàn bộ mã nguồn được lưu trữ công khai tại: \href{https://github.com/coldwind444/dino-fe.git}{Frontend Repository} và \href{https://github.com/boyastro/dino_math_backend.git}{Backend Repository}.
