...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Đối với chức năng Đăng nhập bằng tài khoản hệ thống, người dùng phải nhập thông tin đăng nhập vào các field \textit{Email} và \textit{Mật khẩu} như \textit{Hình \ref{fig:login1}} và \textit{Hình \ref{fig:login2}}. Nếu người dùng quên mật khẩu, có thể bấm vào link \textit{Quên mật khẩu} để truy cập vào trang \textbf{Lấy lại mật khẩu} được mô tả ở mục \ref{subsec:resetpassword}.
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.9\textwidth]{Image/UI/Auth/LoginPage-HidePassword.jpg}
				\vspace{0.6cm}
				\caption{Giao diện đăng nhập - Ẩn mật khẩu}
				\label{fig:login1}
			\end{figure}
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.9\textwidth]{Image/UI/Auth/LoginPage-ShowPassword.jpg}
				\vspace{0.6cm}
				\caption{Giao diện đăng nhập - Hiện mật khẩu}
				\label{fig:login2}
			\end{figure}
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.9\textwidth]{Image/UI/Auth/LoginErrorMessage.jpg}
				\vspace{0.6cm}
				\caption{Giao diện đăng nhập - Lỗi sai mật khẩu}
				\label{fig:wrongpassword}
			\end{figure}
			\item Đối với chức năng đăng nhập bằng Google, người dùng sẽ bấm vào nút \textit{Đăng nhập bằng Google} và được điều hướng đến trang xác thực
			của Google. Sau đó, người dùng thực hiện các thao tác đăng nhập bằng tài khoản Google. Nếu thành công, trang xác thực sẽ điều hướng người dùng
			về trang chủ của mình.
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Bước 2 - Nhập thông tin đăng ký}: Người dùng cần nhập  các thông tin bao gồm: Email, Họ và tên, Mật khẩu và Xác nhận mật khẩu. Đối với thông tin về Họ và tên, thông tin này chỉ xuất hiện khi người dùng chọn đăng ký tài khoản Phụ huynh, còn đối với Học sinh, thông tin này sẽ được bổ sung sau ở giao diện \textbf{Onboarding} được mô tả ở mục \ref{subsec:onboarding}.
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.9\textwidth]{Image/UI/Auth/SignUpPage-SignUp-Student.jpg}
			\vspace{0.6cm}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.9\textwidth]{Image/UI/Auth/SignUpPage-SignUp-Parent.jpg}
			\vspace{0.6cm}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{Image/UI/Auth/EmailExistError.jpg}
		\vspace{0.6cm}
		\caption{Giao diện đăng ký tài khoản - Lỗi email đã được sử dụng}
		\label{fig:regerror}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{Image/UI/Auth/SignUpSuccess.jpg}
		\vspace{0.6cm}
...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{Bước 1 - Gửi OTP qua email}: Người dùng cần nhập email mà mình đã dùng để đăng ký tài khoản vào ô trống. Sau đó bấm \textit{Gửi mã xác thực}, hệ thống sẽ xác thực email được nhập vào và gửi một mã OTP đến email đó. Giao diện của bước 1 được thiết kế như \textit{Hình \ref{fig:sendotp}} bên dưới.
		\begin{figure}[H]
			\centering
		...
		\item \textbf{Bước 2 - Nhập OTP}: Người dùng nhập mã OTP vào các ô trống sau đó bấm \textit{Xác thực}. Nếu mã OTP hợp lệ, tiếp tục thực hiện bước 3.
		...
		\item \textbf{Bước 3 - Đặt lại mật khẩu}: Người dùng nhập mật khẩu mới và xác nhận mật khẩu. Nút \textit{Đổi mật khẩu} chỉ được enable khi:
		\begin{itemize}
			\item Giá trị của tất cả các field khác rỗng.
			\item Mật khẩu có ít nhất 8 ký tự, trong đó phải chứa chữ cái in hoa, chữ thường, số và ký tự đặc biệt.
			\item Mật khẩu và Xác nhận mật khẩu phải trùng khớp.
		\end{itemize}
		Giao diện của bước 3 được thiết kế như \textit{Hình \ref{fig:resetpw}} bên dưới.
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.9\textwidth]{Image/UI/Auth/PasswordReset3.jpg}
			\vspace{0.6cm}
			\caption{Giao diện Lấy lại mật khẩu - Bước 3}
			\label{fig:resetpw}
		\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Topics-G1.jpg}
	\vspace{0.6cm}
	\caption{Trang Bài học - Các chủ đề của Lớp 1 (tương tự với các lớp khác)}
	\label{fig:lesson}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Topics-Locked.jpg}
	\vspace{0.6cm}
...
\textbf{Trang Bài tập} chỉ có thể được truy cập bằng cách bấm vào nút \textit{Làm bài} ở \textbf{Trang Chủ đề} được mô tả ở mục \ref{subsubsec:topic}. Tại trang này người dùng có thể làm các dạng bài tập tương tác như kéo thả, trắc nghiệm, đúng/sai, điền khuyết, lắp ghép (xem chi tiết ở mục \ref{subsec:interact}). Phần tên bài học và danh sách các câu hỏi được hiển thị trên sidebar nằm bên trái giao diện. Người dùng có thể chuyển câu hỏi bằng cách nhấn vào các nút được đánh số thứ tự tương ứng. Người dùng có thể nộp bài bằng cách bấm vào nút \textit{Nộp bài} ở dưới cùng của sidebar. Với mỗi câu làm đúng, người dùng sẽ nhận được một lượng thạch anh nhất định. Giao diện làm bài tập được thiết kế như các hình từ \ref{fig:choice} đến \ref{fig:gr}.
	\begin{figure}[H]
		...
		\begin{minipage}{0.48\textwidth}
			...
			\vspace{0.2cm}
			\caption{Dạng Lắp ghép}
		\end{minipage}
		
		\vspace{0.4cm} % Khoảng cách giữa các hàng
	\end{figure}
	\begin{figure}[H]
...
\textbf{Trang Bài thi} cung cấp giao diện làm bài cho \textbf{Bài kiểm tra đầu vào} (mục \ref{subsubsec:studenthome}) và \textbf{Đấu trường} (mục \ref{subsubsec:arena}). Người dùng có thể chuyển câu hỏi bằng cách bấm vào các nút số ở thanh bên trái của màn hình hoặc bấm các nút điều hướng để chuyển sang câu tiếp theo hoặc trước đó. Giao diện còn hỗ trợ hiển thị thời gian làm bài còn lại cũng như số câu hỏi đã làm trên tổng số câu hỏi. Tất cả các câu hỏi kiểm tra sẽ thuộc dạng trắc nghiệm. Đối với bài thi Đấu trường, do thời gian làm bài dài (7 ngày), người dùng có thể thoát và lưu tiến trình làm bài của mình để có thể tiếp tục làm sau đó.
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Exam.jpg}
		\vspace{0.6cm}
		\caption{Trang Bài thi}
	\end{figure}
...
Trang xếp hạng sẽ hiển thị top 8 học sinh đạt số lượng thạch anh cao nhất (thạch anh là tiêu chí để xếp hạng học tập), đồng thời vinh danh 3 học sinh xuất sắc nhất. Ngoài ra, trang còn hiển thị số lượng thạch anh mà người dùng đang sở hữu và vị trí xếp hạng hiện tại của người dùng.
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Leaderboard.jpg}
		\vspace{0.6cm}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Mission-Complete.jpg}
	\vspace{0.6cm}
	\caption{Trang Nhiệm vụ - Đã hoàn thành}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Mission-Incomplete.jpg}
	\vspace{0.6cm}
...
\newpage
Nhóm đã hiện thực được một minigames với tên gọi Math Match. Người chơi sẽ phải nối các phép tính có kết quả bằng nhau để hoàn thành màn chơi.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/MathMatchGame.jpg}
	\vspace{0.6cm}
...
\vspace{0.1cm}
\newline
Để liên kết tài khoản, học sinh cần nhập mã liên kết vào ô trống (mã liên kết do phụ huynh cung cấp), sau đó bấm gửi yêu cầu và chờ xác nhận của phụ huynh. Sau khi đã liên kết tài khoản, học sinh có thể xem mã liên kết (mã mời) của Family hiện tại và có thể chia sẻ cho các học sinh khác. Ngoài ra, học sinh còn có thể xem đươc phụ huynh nào đang quản lý Family của mình và các thành viên khác trong Family.
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.9\textwidth]{Image/UI/Main-Student/Account-Unlinked.jpg}
			\vspace{0.6cm}
			\caption{Quản lý tài khoản cá nhân - Chưa liên kết tài khoản}
		\end{figure}
		\begin{figure}[H]
			\centering
...
\begin{enumerate}
	...
	\vspace{0.1cm}
	\newline
	Để liên kết với tài khoản của học sinh, phụ huynh có thể làm theo 2 cách sau đây:
		\begin{itemize}
			\item Cách 1: Sao chép mã liên kết và chia sẻ cho học sinh. Sau khi học sinh nhập mã liên kết và gửi yêu cầu, phụ huynh sẽ nhận được email xác nhận, lúc này cần chấp nhận yêu cầu để liên kết tài khoản.
			\item Cách 2: Liên kết trực tiếp bằng cách nhập email của tài khoản học sinh muốn liên kết, sau đó bấm gửi yêu cầu. Học sinh sẽ nhận được lời mời từ email và chấp nhận để liên kết tài khoản.
		\end{itemize}
		
		\newpage
		Sau khi đã liên kết, phụ huynh có thể xem danh sách các tài khoản học sinh đã được thêm vào Family như \textit{Hình \ref{fig:linked}}.
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.9\textwidth]{Image/UI/Main-Parent/NoLinkedAccount.jpg}
			\vspace{0.6cm}
			\caption{Quản lý tài khoản cá nhân - Liên kết tài khoản bằng mã liên kết hoặc liên kết trực tiếp}
		\end{figure}
		\begin{figure}[H]
			\centering
...
\subsection{Mô tả thiết kế hệ thống bài tập}
...
\input{Content/Hệ thống bài tập}
